...
def chonacc():
    json_data = {}
    response = requests.get('https://gateway.golike.net/api/tiktok-account', headers=headers, json=json_data)
    if response.status_code == 200:
        return response.json()
    else:
        print("Lỗi khi gọi API chonacc:", response.text)
        return None
...
def nhannv(account_id):
    ...
    json_data = {}
    response = requests.get('https://gateway.golike.net/api/advertising/publishers/tiktok/jobs',
                            params=params, headers=headers, json=json_data)
    if response.status_code == 200:
        return response.json()
    else:
        print("Lỗi khi gọi API nhannv:", response.text)
        return None
...
def hoanthanh(ads_id, account_id):
    ...
    if response.status_code == 200:
        ...
        print("Lỗi khi gọi API hoanthanh:", response.text)
        return None
...
def baoloi(ads_id, object_id, account_id, loai):
    ...
    response1 = requests.post('https://gateway.golike.net/api/report/send', headers=headers, json=json_data1)
    if response1.status_code != 200:
        print("Lỗi khi gửi báo lỗi:", response1.text)
        return None
    ...
    if response2.status_code == 200:
        ...
        print("Lỗi khi gọi API skip-jobs:", response2.text)
        return None

def dsacc():
    chontktiktok = chonacc()
    if chontktiktok and chontktiktok.get("status") != 200:
...
for i in range(len(chontktiktok["data"])):
    print(
        f'\033[1;97m[ ⟨⟩ ]\033[;36m [{i+1}] \033[1;91m=> \033[1;97mTên Tài Khoản :\033[1;93m {chontktiktok["data"][i]["nickname"]}'
    )
dsacc() 
while True:
  try:
